...
class DeepFM(nn.Module):
    def __init__(self,cate_fea_nuniqs,nume_fea_size=0,emb_size=8,
        ...
        """
        :param cate_fea_nuniqs:类别特征的唯一值个数列表
        :param nume_fea_size: 数值特征的个数
        :param emb_size:
        :param hid_dims:
        :param num_classes:
        :param dropout:
        """
        super().__init__()
        self.cate_fea_nuniqs = len(cate_fea_nuniqs)
        self.nume_fea_size = nume_fea_size

        """FM部分"""
        if self.nume_fea_size != 0:
            self.fm_1st_order_dense = nn.Linear(self.nume_fea_size, 1) # 数值特征的一阶参数
        ...
        """DNN部分"""
        self.all_dims = [self.cate_fea_nuniqs * emb_size] + hid_dims
        self.dense_linear = nn.Linear(self.nume_fea_size, self.cate_fea_nuniqs * emb_size)
        self.relu = nn.ReLU()
        # for DNN
    ...
    def forward(self,X_sparse,X_dense=None):
        ...
        :param X_parse:类别型特征输入 [bs,cate_fea_size]
        :param X_dense: 数值型输入特征  [bs,dense_fea_size]
        :return:
        """
        ...
        fm_1st_sparse_res = torch.sum(fm_1st_sparse_res,dim=1,keepdim=True) # [bs,1]
        ...
        # 先求和再平方
        sum_embed = torch.sum(fm_2nd_concat_1d,dim=1)  # [bs,emb_size]
        square_sum_embed = sum_embed ** 2 # [bx,emb_size]
        # 先平方再求和
        square_embed = fm_2nd_concat_1d ** 2 # [bs,n,emb_size]
        sum_square_embed = torch.sum(square_embed,dim=1)
        # 相减除以2
...
# 定义日志
def write_log(w):
    file_name = 'data/' + datetime.date.today().strftime('%Y%m%d') + "_{}.log".format("deepfm")
